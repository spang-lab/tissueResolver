#' Compute the specific expression and regulation from a tissuemodel
#'
#' @description
#' Computes the specific expression and regulation for a certain group of
#' cells, weighting the individual cells accordingly. Normalization of the
...
#'    Defaults to `TRUE`.
#'
#' @return A tibble holding the expression and regulation of each gene within
#'    each bulk and celltype.
#'
#' @examples
#' library(dplyr)
#' library(tibble)
...
fast_specific_expression_regulation <- function(
  ...
  # get total number of steps required for progress bar
  if (show_pb) {
  ...
  # tibble in order to comprise the type of each cell
    ...
    lapply(split(celldf, celldf[[by]]), function(df_ct) {
      ...
      do.call(rbind,
        lapply(split(df_ct, df_ct[[bulk_id]]), function(df_ct_bulk) {
          ...
          se <- as.matrix(
            sclibrary[genes, cellids]) %*% as.matrix(weights[cellids]
          )
...
#'
#' @description
#' Compute the specific expression and regulation specific for certain groups,
#' weighting the individual cells accordingly. Normalization of the expression
#' column is such that the total expression of a gene is approximately that of
...
#'    tissue to the bulk.
#'
#' @return A tibble holding the expression and regulation of each gene
#'    within each bulk and celltype.
#'
#' @examples
#' library(tibble)
#' set.seed(42)
    ...
    for (run_i in 1:n_boot) {
      ...
      if (run_i == 1) {
        ...
        csre <- csre %>%
          ...
          dplyr::select(
            -expression.x, -expression.y, -regulation.x, -regulation.y
          )
...
#' Compute TOTAL explained expression for quality control
#'
#' @description
#' Computes the TOTAL explained expression, i.e., the specific gene expression
#' within each bulk. I.e., it uses the weights computed in fit tissue to
...
explained_expression <- function(
  ...
  # weigh each cell appearing in the tissuemodel with the corresponding
  # weight computed by fit_tissue then reshape to having the expression of
  # each gene associated to each bulk as separate column then group all
  # entries which belong to the same gene AND the same bulk and compute
  # the sum of expression (explained expression) over this group finally
  # cancel the weight and cell_id column, i.e., se contains bulk_id, gene
  # and explained expression
  se <- cbind(
    celldf,
    celldf %>%
      dplyr::pull(weight) * t(sclibrary[, celldf %>% dplyr::pull(cell_id)])
  ) %>%
    tidyr::pivot_longer(
