#' Plot the cell type specific expression between two bulk groups
#'
#' @description
#' This function generates several bar and heatmap plots for comparing the
#' cell type specific expression between two bulk groups.
#'
#' @param csre An object returned by `specific_expression_regulation()` with
#'    an added `group` column that indicates the bulk group of each bulk.
#' @param heatmapviz The quantity to show in the heat map. Can be set to
#'   ` "logdiffexpr"` (logarithmic differential expression), `"logdiffreg"`
#'    (log differential regulation per cell), `"relchange"` (relative change
#'    in expression compared to the average of the two groups), or
#'    `"relregchange"` (relative change in regulation per cell compared to
#'    the average of the two groups).
#' @param barplotviz The transformation to show in the bar plot for each gene
#'    Can be set to:
#'    - `"linear"`: Show the reconstructed gene expression of the two bulk
#'      groups as a stacked colored bar plot.
#'    - `"log"`: Show the total log-expression in both groups.
#'    - `"logdiff"`: Show the differential log-expression between the two
#'      groups in a single bar plot.
#'    - `"relative"`: Show the group-A expression as 100% fractions and the
#'      group-B relative to that.
#'    - `"relativeB"`: Similarly, but normalized to the group-B expression.
#' @param groupA The identifier of bulk group A (factor in the `group` column).
#' @param groupB The identifier of bulk group B (factor in the `group` column).
#' @param ctypes If not `NULL`, the order in which the cell types will be
#'    displayed. All cell types present in the dataset but not in this array
#'    will be summed as `"other"`.
#' @param addexpressionlevel If set to `TRUE` and `barplotviz = "relative"`,
#'    add an additional bar plot that indicates the expression level of the
#'    gene present in bulk group A.
#'
...
plot_csre <- function(
  ...
  groupavg <- csre %>%
    ...
    dplyr::mutate(celltype = ifelse(
      celltype %in% remove_celltypes, "other", celltype
    )) %>%
  ...
  # the average total explained expression across bulks for each (group,gene)-
  # group this is just the lower part of groupavg but without the regulation
  # column
  ...
  diff <- groupavg %>%
    ...
    # give avgexpr, avgreg, and total_expr separate columns for both
    # bulkgroups A and B
    tidyr::pivot_wider(
      names_from = group,
    ...
    dplyr::rename(
      avgexpr_A = paste0("avgexpr_group_", groupA),
      avgexpr_B = paste0("avgexpr_group_", groupB),
      avgregulation_A = paste0("avgregulation_group_", groupA),
      avgregulation_B = paste0("avgregulation_group_", groupB),
      total_expr_A = paste0("total_expr_group_", groupA),
      total_expr_B = paste0("total_expr_group_", groupB)
    ) %>%
    # for each row (comprising gene,celltype) give logp1 difference of
    # avgexpr, avgreg and total_expr
    dplyr::mutate(
      logdiffexpr = log2((avgexpr_A + 1.0) / (avgexpr_B + 1.0)),
      logdiffreg = log2((avgregulation_A + 1.0) / (avgregulation_B + 1.0)),
      logdifftotal = log2((total_expr_A + 1.0) / (total_expr_B + 1.0)),
      # give relative difference between avgexpr relative to the total_expr
      relchange = 0.5 * (avgexpr_A - avgexpr_B) / (total_expr_A + total_expr_B),
      # give relative difference between avgregulation
      relregchange = 0.5 * (avgregulation_A - avgregulation_B) /
        (avgregulation_A + avgregulation_B),
      # replace NAs by 0
      relregchange = ifelse(is.na(relregchange), 0.0, relregchange)
    )

  # for the column specified in heatmapviz (e.g., "relchange") we compute
  # the absolute value 5% and 95% quantile then we take the maximum of both
  # values and scale it with 1.1. This quantity will be used for heatmap
  # plotting at the end
  limits <- 1.1 * max(abs(
    diff %>%
      dplyr::pull(heatmapviz) %>%
      stats::quantile(probs = c(0.05, 0.95), na.rm = TRUE)
  ))
  ...
  tmp <- diff %>%
    ...
    tidyr::drop_na() %>%
    # give each gene a separate column storing the heatmapviz value for each
    # celltype
    tidyr::pivot_wider(
      id_cols = celltype,
      names_from = gene,
    ...
    # only consider the actual celltypes so neither "other" nor
    # "total_explained"
  ...
  ord_genes <- colnames(m)[
    stats::hclust(stats::dist(t(m)), method = "median")$order
  ]

  if (is.null(ctypes)) {
    # hierarchically order ctypes
    ctypes <- rownames(m)[
      stats::hclust(stats::dist(m), method = "median")$order
    ]
    ...
    # if ctypes is not null keep the ordering of ctypes and add "other"
    # to the end
    if (
      !("other" %in% ctypes) &&
        ("other" %in% (diff %>% dplyr::pull(celltype) %>% unique()))
    ) {
      ctypes <- c(ctypes, "other")
    }
  }
  ...
  if (barplotviz == "linear") {
    groupavg <- groupavg %>%
      ...
      dplyr::mutate(
        dplyr::across(group, \(x) factor(x, levels = c(groupA, groupB)))
      )

    exprchanges1 <- groupavg %>%
      # create for each bulk group a barplot, where for each gene the
      # avgexpression contribution of each cell type
      # is visualized by the filling of the bar
      ggplot2::ggplot(
        ggplot2::aes(
          x = avgexpr,
          y = factor(gene, rev(ord_genes)),
          fill = factor(celltype, levels = ctypes)
        )
      ) +
    ...
    # create for each bulk group a barplot, where for each gene the logp1
    # avg total expression is visualized by the length of the bar
    exprchanges1 <- plotdf %>%
      ggplot2::ggplot(
        ggplot2::aes(x = logexpr, y = factor(gene, rev(ord_genes)))
      ) +
      ggplot2::geom_bar(stat = "identity", position = "stack") +
      ggplot2::facet_wrap(~group, strip.position = "left") +
      ggplot2::ylab(NULL) +
    ...
    exprchanges1 <-
      ...
      ggplot2::ggplot(
        ggplot2::aes(x = logdiff, y = factor(gene, rev(ord_genes)))
      ) +
      ggplot2::geom_bar(stat = "identity", position = "stack") +
      ggplot2::ylab(NULL) +
      ggplot2::xlab(NULL) +
    ...
    # fraction between the avgexpr of each celltype relative to the
    # total expression cumulated across celltypes within the bulkgroup
    # specified in barplotviz ("relative" defaults to "relativeA")
    plotdf <- groupavg %>%
      dplyr::select(-avgregulation) %>%
      dplyr::filter(celltype != "total_explained") %>%
      ...
      dplyr::rename(
        group_A = dplyr::all_of(groupA),
        group_B = dplyr::all_of(groupB)
      ) %>%
      dplyr::group_by(gene) %>%
    ...
    # as specified in the barplotpviz parameter the fractions are relative to
    # one of both groups, justifying the equal length of one of the barplots
    exprchanges1 <- plotdf %>%
      ggplot2::ggplot(
        ggplot2::aes(
          x = frac,
          y = factor(gene, rev(ord_genes)),
          fill = factor(celltype, levels = rev(ctypes))
        )
      ) +
      ggplot2::geom_bar(stat = "identity", position = "stack") +
      ggplot2::facet_wrap(~group, strip.position = "left") +
    ...
    if (addexpressionlevel) {
      ...
      #join both plots
      exprchanges1 <- gridExtra::grid.arrange(
        exprchanges1,
        exprlevelplot,
        ncol = 2,
        widths = c(0.8, 0.2)
  ...
  # each tile corresponds to one gene and the color expresses the
  # relchange stored in diff relchange is the relative change
  # between both bulkgroups in avgexpr
  exprchanges2 <- diff %>%
    dplyr::filter(celltype != "total_explained") %>%
    ggplot2::ggplot(
      ggplot2::aes(
